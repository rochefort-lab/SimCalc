...
def spike_data_poisson(time=1000., dt=.1, rates=[0.], trials=1):
    ...
    for t in range(int(trials)):
        ...
        for r in rates:
            sp_prob = r*dt
            sp_data = np.concatenate( (sp_data, np.random.binomial(1, sp_prob, len(trng)) ) )
...
def gen2photon(calcium,bg,X,R,l,sigma_noise,freq):
    ...
    # setup output
    out  =zeros((l,l,len(calcium[0,:])))  # this is where we'll store the video
    ...
    for i in range(numbers[0]):
        mean = X[i,0],X[i,1]
        var  = R[i]
        filt = makeGaussianFilter(mean[0],mean[1],[[var,0],[0,var]],l,False)
        filt[filt>0.5]+=0.1
    ...
    for i in range(count,count+numbers[1]):
        ...
        var  = R[count]
        filt = makeGaussianFilter(mean[0],mean[1],np.array([[var/2,0],[0,var/2]]),l,True)
        filt[filt>0.65]+=0.1
        for t in range(len(calcium[0,:])):
    ...
    for i in range(count,rlen):
        ...
        sigma = R[count]/100
        path,filt = makeDendriteFilter(X[count,0],X[count,1],length,sigma,l,rand()*np.pi*2,0.05)
        filt[filt>0.5]+=0.1
    ...
    for i in range(len(x)):
        bgfilt += makeGaussianFilter(x[i],y[i],[[randint(60*l,80*l),0],[0,randint(60*l,80*l)]],l)
    # normalize filter
    bgfilt /= bgfilt.max()
    bgfilt += 1
    # bgfilt /= bgfilt.max()
